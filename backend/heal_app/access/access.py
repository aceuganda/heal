...
from heal_app.access.models import DocumentAccess
from heal_app.configs.constants import PUBLIC_DOC_PAT
from heal_app.db.document import get_acccess_info_for_documents
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "heal_app.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User | None, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "heal_app.access.access", "_get_acl_for_user"
    )
